...
**===================================**\
**REMINDER**\
**ICDPICR Version 2.0.1 IS BEING TESTED**\
...
Version 2.0.1 of “icdpicr”, named “icdpicr2”, is a further update in response to these studies and numerous other inquiries and suggestions.  Because of copyright restrictions, it continues to use only the original version of the Abbreviated Injury Score (AIS).[7]  


### The most important changes from Version 1 to Version 2 are as follows:

* Version 1.0.0 of “icdpicr” used TQIP and NIS data from 2017 and 2016.  Version 2.0.0 (“icdpicr2”) uses data from 2020.  Version 2.0.1 makes no attempt to accommodate ICD-9 data and requires that data be in ICD-10 format.

* Version 1.0.0 of “icdpicr” allowed the user to specify whether data were in ICD-10-CM or a basic ICD-10 format.  Version 2.0.1 (“icdpicr2”) combines all ICD-10-CM and other simpler ICD-10 codes (including those previously omitted) in the same table.  

* The “ROCmax” option in Version 1.0.0 of “icdpicr” replaced the original *ad hoc* algorithm with the well-established methodology of ridge regression to estimate the independent effect of each injury diagnosis.  Version 1.0.0 also allowed the user to choose either the TQIP PUF or the NIS as the reference database.  Version 2.0.1 (“icdpicr2”) only uses the ROCmax method, again based on ridge regression.  However, it computes an average of the AIS obtained from TQIP and the AIS obtained from NIS. 
 
* In response to user feedback, Version 2.0.1 (“icdpicr2”) also modifies the AIS assignments to make them more internally consistent (e.g., a right-sided injury has the same AIS as an otherwise identical left-sided injury).  The lookup table of injury codes now also includes valid codes not in either reference database (including basic or truncated ICD-10 codes) and AIS values are assigned to these codes using a weighted average of similar codes.  Where possible, ICD-10 injury codes published by the WHO now have AIS assigned directly based upon the data gathered from seven different countries as part of an International Collaborative Effort on Injury Statistics.[8] 

* As before, Version 2.0.1 (“icdpicr2”) reports Maximum AIS for each body region and overall, Injury Severity Score (ISS),[9] and “New Injury Severity Score”,[10] and now reports the mortality predictions both from TQP and from NIS for each individual.

* Version 2.0.1 (“icdpicr2”) reports the CDC mechanism and intent categories using the most recent versions.[11]  It computes two versions (“multiplicative injury” and “worst injury”) of an “ICD-based Injury Severity Score” (ICISS).[8,12]   It adds the CDC framework for body area and nature of injury, and calculates an approximate survival probability based on the empirical survival for each cell of the framework.[13-14]  

* Version 2.0.1 (“icdpicr2”) has also been simplified in several ways so that it will run faster than previous versions. In particular, the extraction of injury mechanism codes has been made much more efficient, which greatly reduces the most time-consuming step in Version 1.0.0.


### **The following programs were used to develop Version 2.0.1 (“icdpicr2”):**
...
* **icd10aisD** (New in Version 2.0.1) – Modifies the AIS scores obtained by icd10aisC in the following ways:
...
* **i10_map_sev:** This table is new in Version 2.0.1, replacing i10_map_roc.  It uses the results of icd10aisC and icd10aisD to produce an approximate AIS score and body region for any ICD-10 code and effect estimates from the ridge regression for any ICD-10-CM code contained either in TQP or NIS.

* **i10_map_mech:**  This table is new in Version 2.0.1, replacing i10_ecode.  It uses the results of icd10aisE to assign a mechanism category for any ICD-10 external cause of injury code (starting with letters T, V, W, X, or Y), using a table proposed by the CDC.[11]

* **i10_map_iciss:**  This table is new in Version 2.0.1.  It uses the results of icd10aisD to assign ICD-10 injury codes to a 4-digit ICD-10 code and its associated survival probability as published by Gedeborg and colleagues.[8]

* **i10_map_frame:**  This table is new in Version 2.0.1.  It uses the results of icd10aisE to assign ICD-10 injury codes to a cell in the “Framework for Presenting Injury Data” published by Hedegaard and colleagues.[13]
...
* **Program “framework”**
Reads in user data in the specified format.  Assigns ICD-10 injury codes to a cell in the CDC “Framework for Presenting Injury Data”.[13]  Calculates a predicted survival for each case.  Further details about the available options are provided in the help file for this function.
